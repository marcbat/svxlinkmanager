        ...
        public virtual async Task HandleValidSubmit(string redirect)
        {
            ...
            switch (Channel)
            {
                case SvxlinkChannel svxlinkChannel:
                    await Mediatr.Send(new UpdateSvxlinkChannelCommand(Options.Value.ConfigId, svxlinkChannel.Id, svxlinkChannel.Name,svxlinkChannel.Host, svxlinkChannel.CallSign, svxlinkChannel.AuthKey, svxlinkChannel.Port, svxlinkChannel.ReportCallSign, svxlinkChannel.Sound?.SoundName, svxlinkChannel.Sound?.SoundFile), CancellationToken.None);
